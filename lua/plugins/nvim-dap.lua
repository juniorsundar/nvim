return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>LAp", function()
      dap.set_breakpoint(nil, nil, vim.fn.input "Log point message: ")
    end, { desc = "Set Log Point" })
    vim.keymap.set("n", "<leader>LAr", function()
      dap.repl.open()
    end, { desc = "Open REPL" })
    vim.keymap.set("n", "<leader>LAl", function()
      dap.run_last()
    end, { desc = "Run Last" })
    vim.keymap.set("n", "<leader>LAo", function()
      dap.step_over()
    end, { desc = "Step Over" })
    vim.keymap.set("n", "<leader>LAi", function()
      dap.step_into()
    end, { desc = "Step Into" })
    vim.keymap.set("n", "<leader>LAu", function()
      dap.step_out()
    end, { desc = "Step Out" })
    vim.keymap.set("n", "<leader>LAt", function()
      dap.terminate()
    end, { desc = "Terminate" })
